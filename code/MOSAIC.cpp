...
void MOSAIC::Initialize(int size_template_x_, int size_template_y_, int grid_x_, int grid_y_,
                        int offset_templates_, std::vector<cv::Mat> Template_set_ptr,
                        std::vector<cv::Mat> Mask_set_ptr, cv::Mat *New_visibility_map_)
{
	// Inits
...
//    if(Template_set_ptr->channels()>1)
//        isGray = false;
//    else
//        isGray = true;
...
void	MOSAIC::Process()
{
	...
	for(int v=0; v<grid_x; v++)
	{
		...
		{
            if(New_visibility_map->at<uchar>(u,v) == 1)
			{
				// Now set pointers to current template
                cv::Mat	Template = Template_set[counter];
                cv::Mat	Mask = Mask_set[counter];
...
void	MOSAIC::Process(std::vector<int> active_templates)
{
	// Switch to unused buffer 
	//displayMosaic = &Mosaic[switch_buffer];
    //switch_buffer = !switch_buffer;
...
void	MOSAIC::PrintMosaic(std::string location)
{
    std::cout << "location: " << location << std::endl; //TODO remove
    cv::imwrite(location, Mosaic);
}
...
void	MOSAIC::PrintMosaic(int counter)
{
    ...
    text << "../storage/output_" << counter << ".png";
